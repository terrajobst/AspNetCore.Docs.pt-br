---
title: Proteger os aplicativos de servidor do mais ASP.NET Core
author: guardrex
description: Saiba como mitigar as ameaças de segurança para aplicativos de servidor mais incrivelmente.
...
* Trate qualquer método .NET exposto ao JavaScript como você faria com um ponto de extremidade público para o aplicativo.
  ...
  * Leve em consideração que os métodos estáticos e de instância podem ser expostos a clientes JavaScript. Evite compartilhar o estado entre as sessões, a menos que o design chame o estado de compartilhamento com as restrições apropriadas.
    * Para métodos de instância expostos `DotNetReference` por meio de objetos criados originalmente por injeção de dependência (di), os objetos devem ser registrados como objetos com escopo. Isso se aplica a qualquer serviço de DI que o aplicativo de servidor mais incrivelmente usa.
...
Se um retorno de chamada de evento invocar uma operação de execução demorada, como buscar dados de um serviço ou banco de dado externo, considere usar uma proteção. A proteção pode impedir que o usuário enfileirar várias operações enquanto a operação está em andamento com comentários visuais. O código de componente a `isLoading` seguir `true` define `GetForecastAsync` enquanto obtém dados do servidor. Enquanto `isLoading` estiver`true`, o botão será desabilitado na interface do usuário:
...
Alguns eventos dom, `oninput` como ou `onscroll`, podem produzir uma grande quantidade de dados. Evite usar esses eventos em aplicativos de servidor mais incrivelmente.

## <a name="additional-security-guidance"></a>Diretrizes de segurança adicionais

As diretrizes para proteger os aplicativos ASP.NET Core se aplicam a aplicativos de servidor mais podestas e são abordadas nas seguintes seções:
...
O servidor mais incrivelmente usa o Signalr para comunicação entre o cliente e o servidor. O servidor de mais incrivelmente normalmente usa o transporte que o Signalr negocia, que normalmente é WebSockets.

O servidor mais incrivelmente não garante a integridade e a confidencialidade dos dados enviados entre o servidor e o cliente. Sempre use HTTPS.
...
A estrutura de servidor mais incrivelmente toma as medidas para se proteger contra algumas das ameaças anteriores:
