...
**DICAS** Implemente apenas as classes que você pretende usar em seu aplicativo.
...
Você pode criar uma `RoleStore` classe que fornece os métodos para todas as operações de dados em funções. Essa classe é equivalente à classe [RoleStore&lt;TRole&gt; ](/dotnet/api/microsoft.aspnetcore.identity.entityframeworkcore.rolestore-1) . Na classe, você implementa a `IRoleStore<TRole>` e, opcionalmente, `IQueryableRoleStore<TRole>` a interface. `RoleStore`
...
* [Identidade de ASP.NET Core](https://github.com/aspnet/identity) &ndash; Esse repositório inclui links para provedores de loja mantidos pela Comunidade.
