...
Problemas conhecidos para projetos direcionados para uma estrutura compartilhada:
...
* Exclua *Areas/Identity/IdentityHostingStartup.cs*.
* Atualizar o  *\_Loginpartial* arquivo:
  ...
  * Atualizar o `<form />` elemento para `<form asp-area="Identity" asp-page="/Account/Logout" asp-route-returnUrl="@Url.Page("/Index", new { area = "" })" method="post" id="logoutForm" class="navbar-right">`.
...
* Na *áreas/identidade/páginas/\_viewstart. cshtml*, altere `Layout = "/Pages/_Layout.cshtml"` para `Layout = "/Pages/Shared/_Layout.cshtml"`.
* O  *\_layout. cshtml* arquivo tem as seguintes alterações:
