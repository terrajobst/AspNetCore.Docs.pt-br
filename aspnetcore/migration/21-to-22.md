...
Projetos direcionados ao .NET Core devem usar o [TFM](/dotnet/standard/frameworks) de uma versão maior ou igual a 2.2 do .NET Core. No arquivo de projeto, atualize o `<TargetFramework>` texto interno do nó com `netcoreapp2.2`:
...
Para adotar a [modelo de hospedagem em processo para o IIS](xref:host-and-deploy/iis/index#in-process-hosting-model), adicione o `<AspNetCoreHostingModel>` propriedade com um valor de `InProcess` para um `<PropertyGroup>` no arquivo de projeto:
...
Para projetos que usam um personalizado *Web. config* arquivo na raiz do projeto para gerar seus publicado *Web. config* arquivo:

* No `<handlers>` entrada que adiciona o módulo do ASP.NET Core (`name="aspNetCore"`), altere o `modules` o valor do atributo `AspNetCoreModule` para `AspNetCoreModuleV2`.
* No `<aspNetCore>` elemento, adicione o atributo de modelo de hospedagem (`hostingModel="InProcess"`).

Para obter mais informações e exemplo *Web. config* arquivos, consulte <xref:host-and-deploy/aspnet-core-module#configuration-with-webconfig>.
