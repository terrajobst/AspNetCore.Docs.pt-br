...
* O MVC `Newtonsoft.Json` (Json.NET) dá suporte &ndash; suporte para o uso do MVC com `Newtonsoft.Json` agora faz parte de [Microsoft. AspNetCore. Mvc. NewtonsoftJson](https://www.nuget.org/packages/Microsoft.AspNetCore.Mvc.NewtonsoftJson).
...
* `IHostingEnvironment` `IWebHostEnvironment`, consulte [este comunicado do GitHub](https://github.com/dotnet/AspNetCore/issues/7749).
...
O serializador JSON padrão para ASP.NET Core agora é <xref:System.Text.Json>, que é novo no .NET Core 3,0. Considere o uso de `System.Text.Json` quando possível. Ele é de alto desempenho e não requer uma dependência de biblioteca adicional. No entanto, como `System.Text.Json` é novo, no momento, pode haver recursos ausentes que seu aplicativo precisa. Para obter mais informações, consulte [como migrar de Newtonsoft. JSON para System. Text. JSON](/dotnet/standard/serialization/system-text-json-migrate-from-newtonsoft-how-to).

### <a name="use-newtonsoftjson-in-an-aspnet-core-30-opno-locsignalr-project"></a>Usar Newtonsoft. JSON em um projeto de SignalR ASP.NET Core 3,0
...
* No cliente, encadear uma chamada de método `AddNewtonsoftJsonProtocol` para a instância `HubConnectionBuilder`:
...
* No servidor, ligue uma chamada de método `AddNewtonsoftJsonProtocol` para a chamada do método `AddSignalR` no `Startup.ConfigureServices`:
...
* Atualize `Startup.ConfigureServices` para chamar `AddNewtonsoftJson`.
  ...
  * `AddControllers`
  ...
  `Newtonsoft.Json` configurações podem ser definidas na chamada para `AddNewtonsoftJson`:
...
* Se o aplicativo implementa `IRouter` ou herda de `Route`, use [DynamicRouteValuesTransformer](https://github.com/dotnet/AspNetCore.Docs/issues/12997) como a substituição.
...
A autorização também pode ser configurada para classes específicas de pontos de extremidade. O código a seguir é um exemplo de conversão de um aplicativo MVC que configurou um `AuthorizeFilter` global para um aplicativo com uma política específica que requer autorização:

[!code-csharp[](22-to-30/samples/Startup.cs?name=snippet&highlight=8-9,21-22,26-27,53-54)]

As políticas também podem ser personalizadas. A `DefaultPolicy` está configurada para exigir autenticação:
...
Conforme explicado na documentação sobre [as diferenças de versões anteriores do roteamento](xref:fundamentals/routing#differences-from-earlier-versions-of-routing), há algumas diferenças na geração de link (usando `Url.Link` e APIs semelhantes, por exemplo). Eles incluem:
...
Se você estiver usando [recursos do Newtonsoft. JSON que não têm suporte em System. Text. JSON](/dotnet/standard/serialization/system-text-json-migrate-from-newtonsoft-how-to), poderá voltar para `Newtonsoft.Json`. Consulte [usar o Newtonsoft. JSON em um projeto ASP.NET Core 3,0 SignalR](#use-newtonsoftjson-in-an-aspnet-core-30-signalr-project) anteriormente neste artigo.
...
O sistema de validação no .NET Core 3,0 e posterior trata parâmetros não anuláveis ou propriedades associadas como se tivessem um atributo `[Required]`. Para obter mais informações, consulte o [atributo [Required]](xref:mvc/models/validation?view=aspnetcore-3.1#required-attribute).
