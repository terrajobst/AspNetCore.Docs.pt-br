...
A alteração mais significativa do `WebHostBuilder` à `HostBuilder` está em [injeção de dependência (DI)](xref:fundamentals/dependency-injection). Ao usar `HostBuilder`, você apenas pode injetar <xref:Microsoft.Extensions.Configuration.IConfiguration>, <xref:Microsoft.Extensions.Hosting.IHostingEnvironment>, e <xref:Microsoft.AspNetCore.Hosting.IHostingEnvironment> no construtor da inicialização. O `HostBuilder` DI restrições:
