...
* Se o aplicativo usar cenários [CORS](xref:security/cors) , como `[EnableCors]`, coloque a chamada para `UseCors` antes de qualquer outra middleware que use CORS (por exemplo, Place `UseCors` antes `UseAuthentication`, `UseAuthorization`e `UseMvc`).
* Substituir `IHostingEnvironment` `using` `Microsoft.Extensions.Hosting` por `IWebHostEnvironment` e adicionar uma instrução para o namespace.
...
Se o aplicativo usar um `AuthorizeFilter` filtro global no MVC, é recomendável refatorar o código para fornecer uma política para o `UseAuthorization` middleware.

No exemplo a seguir, uma política personalizada a ser aplicada a todas as solicitações `UseAuthorization` quando é chamada, e `HomeController` o permite o acesso sem o usuário entrar no aplicativo:
...
No exemplo a seguir, `UseAuthorization` processa a autorização sem uma política padrão, mas `/healthz` o ponto de extremidade de verificação de integridade exige que `admin` o usuário esteja na função: 
...
### <a name="migrate-signalr"></a>Migrar sinalizador
...
### <a name="migrate-mvc-controllers"></a>Migrar controladores MVC
...
### <a name="migrate-health-checks"></a>Migrar verificações de integridade
...
## <a name="update-signalr-code"></a>Atualizar o código do sinalizador
...
Em 3,0, a compilação em tempo de execução é um cenário de aceitação. Para habilitar a compilação em tempo https://docs.microsoft.com/aspnet/core/mvc/views/view-compilation?view=aspnetcore-3.0#runtime-compilation de execução, consulte.
