...
* Alguns assemblies foram removidos do `Microsoft.AspNetCore.App` entre 2. x e o 3.0. Talvez você precise adicionar `<PackageReference>` itens se você estiver usando as APIs dos pacotes listados em [aspnet/AspNetCore #3755](https://github.com/aspnet/AspNetCore/issues/3755).
...
* Adicione [Json.NET suporte](#json).

* Projetos como padrão o [modelo de hospedagem em processo](xref:host-and-deploy/aspnet-core-module#in-process-hosting-model) no ASP.NET Core 3.0 ou posterior. Você pode também remover os `<AspNetCoreHostingModel>` elemento se seu valor é `InProcess`.
