...
* Se o aplicativo usar recursos de autenticação/autorização, `AuthorizePage` como `[Authorize]`ou, coloque a chamada `UseAuthentication` para `UseAuthorization` e **depois** `UseRouting`.
* Se o aplicativo usar recursos [CORS](xref:security/cors) , `[EnableCors]`como, coloque `UseCors` avançar.
