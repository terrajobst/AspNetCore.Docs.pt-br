...
* Defina as [Framework Moniker (TFM) de destino](/dotnet/standard/frameworks) para `netcoreapp3.0`:
...
* Atualização `Startup.ConfigureServices` chamar `AddNewtonsoftJson`.
  ...
  `AddNewtonsoftJson` é compatível com os novos métodos de registro de serviço do MVC:
...
No 3.0, a compilação de tempo de execução é um cenário de aceitação. Para habilitar a compilação de tempo de execução, consulte https://docs.microsoft.com/aspnet/core/mvc/views/view-compilation?view=aspnetcore-3.0#runtime-compilation.
