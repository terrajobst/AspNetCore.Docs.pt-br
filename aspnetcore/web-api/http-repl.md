...
* Usado para fazer solicitações HTTP para testar as APIs Web do ASP.NET Core e exibir os resultados.
...
Uma [ferramenta global do .NET Core](/dotnet/core/tools/global-tools#install-a-global-tool) é instalada do pacote NuGet [dotnet-httprepl](https://dotnet.myget.org/feed/dotnet-core/package/nuget/dotnet-httprepl
) hospedado no MyGet.
...
`<BASE URI>` é o URI de base para a API Web. Por exemplo:
...
## <a name="point-to-the-swagger-document-for-the-web-api"></a>Apontar para o documento Swagger da API Web

Para inspecionar corretamente a API Web, defina o URI relativo do documento Swagger da API Web. Execute o seguinte comando:
